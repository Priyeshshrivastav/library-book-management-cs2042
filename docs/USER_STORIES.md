# User Stories

## Sprint 1
1. **Add Book**: As a librarian, I want to add books to the system providing ID, title, and author, so that I can track the collection.
   - *Acceptance Criteria*:
     - Book added successfully with correct details.
     - Error raised if duplicate Book ID is entered.

## Sprint 2
2. **Borrow Book**: As a member, I want to borrow a book by ID so that I can read it.
   - *Acceptance Criteria*:
     - Status changes to 'borrowed'.
     - Error if book not found.
     - Error if book already borrowed.

3. **Return Book**: As a member, I want to return a borrowed book so that others can read it.
   - *Acceptance Criteria*:
     - Status changes to 'available'.
     - Error if book not found.

## Sprint 3
4. **Generate Report**: As a librarian, I want to generate a report of all books and their statuses so I can manage inventory.
   - *Acceptance Criteria*:
     - Report contains all books.
     - Report includes ID, Title, Author, Status.
     - Report has a header row.
